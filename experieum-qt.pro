...
# for boost 1.37, add -mt to the boost experieumries
...
# Dependency experieumry locations can be customized with:
...
windows:!contains(MINGW_THREAD_BUGFIX, 0) {
    ...
    DEFINES += MAC_OSX MSG_NOSIGNAL=0 BOOST_NO_CXX11_SCOPED_ENUMS
    QMAKE_CXXFLAGS += -stdlib=libc++
macx:ICON = src/qt/res/icons/experieum.icns
    QMAKE_CFLAGS_THREAD += -pthread
    QMAKE_CXXFLAGS_THREAD += -pthread


# Set experieumries and includes at end, to use platform-defined defaults if not overridden
...
contains(RELEASE, 1) {
    !windows:!macx {
        # Linux: turn dynamic linking back on for c/c++ runtime experieumries
        LIBS += -Wl,-Bdynamic
    }
}
